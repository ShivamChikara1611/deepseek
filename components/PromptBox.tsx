...
const PromptBox = ({setIsLoading, isLoading}) => {
    ...
    return (
        <form className={`w-full ${false ? 'max-w-3xl' : 'max-w-2xl'} bg-[#404045] p-4 rounded-3xl mt-4 transition-all`}>
            <textarea
            className='outline-none w-fill resize-none overflow-hidden break-words bg-transparent'
            rows={2}
            ...
            <div className='flex items-center justify-between text-sm'>
                ...
                <Image className='w-3.5 aspect-square' alt='' src={prompt ? assets.arrow_icon : assets.arrow_icon_dull}/>
                </button>
                </div>
            </div>
        </form>
